...
contract ARopstenConsumer is Chainlinked, Ownable {
  ...
  event RequestCarbonFulfilled(
    bytes32 indexed requestId,
    uint256 indexed carbon
  );
  ...
  // Changed
  function fulfillCarbon(bytes32 _requestId, uint256 _carbon)
    public
    recordChainlinkFulfillment(_requestId)
  {
    emit RequestCarbonFulfilled(_requestId, _carbon);
    carbon = _carbon;
  }
